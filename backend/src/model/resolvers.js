...
const { res, myQueries } = require("./index.js");
...
const resolvers = {
  ...
  Mutation: {
    newUser: async (_, args) => {
      ...
      async function checkUser(users, password) {
        if (users && !password) {
          ...
          let response = await res(myQueries.searchUser, [
            args.username,
            args.password,
          ]);
          if (response[0]) {
      ...
      if (args.status == "test") {
        ...
        if (await res) {
          let token = jwt.sign({ user_id: await res[0].user_id }, varPassword);
          return await token;
        } else return null;
      } else if (args.status == "create") {
        if ((await checkUser(await response)) === false) {
          let response = await res(myQueries.newUser, [
            args.username,
            args.password,
          ]);
          //console.log(await response[0].user_id);
          let token = jwt.sign(
            { user_id: await response[0].user_id },
            varPassword
          );
          return await token;
        } else return null;
    ...
    newTask: async (_, args) => {
      if (args.status.toLowerCase() == "test") {
        console.log(args);
      } else if (args.status.toLowerCase() == "create") {
        let obj = jwt.verify(args.token, varPassword);
        let response = await res(myQueries.newTask, [
          obj.user_id,
          args.TaskText,
          fullDate,
          args.TaskDeadline,
    ...
    setTask: async (_, args) => {
      ...
      if (args.delete == 1) {
        let response = await res(myQueries.deleteTask, [
          obj.user_id,
          args.taskId,
        ]);
        return await response;
      } else {
        let response = await res(myQueries.setTaskText, [
          args.text,
          obj.user_id,
          args.taskId,
        ]);
