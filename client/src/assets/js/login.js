...
async function fetchData(username, password, status){
	let response = await fetch(`http://localhost:4000`, {
		...
		body: JSON.stringify({query, variables:{user: username, password, status} } )
	});
...
form.addEventListener('submit', async (event) => {
	...
	let s
	if(!( regSelect.classList.value.includes('selected') ) && loginSelect.classList.value.includes('selected') ){
		s = "login"
	}else{
		s = "create"
	}
	fetchData(logInput.value, passInput.value, s)
		.then( data => {
	...
	if(!( regSelect.classList.value.includes('selected') ) && loginSelect.classList.value.includes('selected') ){
		let response = await fetchData( logInput.value, passInput.value, "login");
